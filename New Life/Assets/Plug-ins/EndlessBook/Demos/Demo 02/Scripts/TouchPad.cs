namespace echo17.EndlessBook.Demo02
{
    ...
    public class TouchPad : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (Input.GetMouseButtonDown(0) && Chapter3.Instance.isSafe)
            {
                // left mouse button pressed
                DetectTouchDown(Input.mousePosition);
            }
            if (Input.GetMouseButtonUp(0) && Chapter3.Instance.isSafe)
            {
                // left mouse button un-pressed
                DetectTouchUp(Input.mousePosition);
            }
            else if (touchDown && Input.GetMouseButton(0) && Chapter3.Instance.isSafe)
            {
                // dragging
